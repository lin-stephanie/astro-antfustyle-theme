...
| `tags`                | `string[]` (`[]`)                                 | Adds tags to the post. These are displayed in post metadata and can also be used by list/card view tag filters on pages that enable them.                                                                                                                      |
| `cover`               | `image() \| string` (`''`)                        | Defines a cover image for the post. You can provide either a local image path relative to the current file or a remote image URL.                                                                                                                              |
| `coverAlt`            | `string` (`''`)                                   | Defines the cover image alt text. If `UI.postView.useCoverAltAsCaption` is `true`, it is also shown below the cover image as a caption.                                                                                                                        |
...
| `minutesRead`         | `number \| boolean` (`true`)                      | Provides an estimated reading time. Set a number to override it manually, set `true` or delete the field to auto-generate it, and set `0` or `false` to hide it.                                                                                               |
...
| `redirect`            | `string \| ''` (`''`)                             | Defines a URL to redirect the post. Leave it as an empty string or delete it if no redirection is needed.                                                                                                                                                      |
