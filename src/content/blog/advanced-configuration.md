...
This post is an advanced guide on customizing the [Astro AntfuStyle Theme](https://github.com/lin-stephanie/astro-antfustyle-theme). If you have not yet configured the `src/config.ts` file, it is recommended to first review ==Basic Configuration==.
...
You may need to adjust the SVG element's `width`, `height`, and `viewBox` attributes to fit properly. It's recommended to paste the code below into the [Satori - OG Image Playground](https://og-playground.vercel.app/) under the Tailwind (experimental) tab for testing (set parameters as shown below).
...
> [!warning]- Adjust black elements in your logo
> 
...
> [!note]- If you don't need PWA support, you can delete `manifest.webmanifest.js`, `icon-192.png`, `icon-512.png`, and `icon-mask.png`.
...
- [UnoCSS](https://unocss.dev/) utility classes (configured with :link{id=@unocss/preset-uno}, compatible with :link[Tailwind CSS]{id=tailwindlabs/tailwindcss} and :link[Windi CSS]{id=windicss/windicss})
...
```astro title='src/components/base/Footer.astro' ins={13-20} collapse={1-8,22-29}
---
...
- Delete the `public/docs` directory (which only contains images for `README.md`).
